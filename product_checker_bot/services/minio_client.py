...
class MyMinioClient:
    def __init__(self, credentials: str = "data/credentials.json") -> None:
        try:
            ...
            self._client = minio.Minio(**credentials)
    ...
    def put_new_photo(
        self, image: BytesIO, length: int, user: str, object_name: str
    ...
    def put_new_bytearray_photo(
        self, bytearray_photo: bytearray, user: str, object_name: str
    ) -> minio.api.ObjectWriteResult:
        ...
        try:
            ...
            minio_res = self.put_new_photo(
                data_stream, len(data_bytes), user, object_name
            )
            return minio_res
        except (AttributeError, TypeError, ValueError):
            raise ValueError(
                "Something goes wrong while uploading byterrary photo to minio!"
            )
